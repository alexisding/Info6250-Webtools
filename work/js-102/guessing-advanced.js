...
// store the whole wordList by initial letter in a map, then pick guess from the same initial letter list
function thinkAbout( wordInfo ) {
  ...
  // EDIT BELOW THIS
  let wordMap = new Map();
  for (let word of wordInfo.allWords) {
  ...
  return wordMap.get(wordInfo.word.charAt(0));
  // EDIT ABOVE THIS
}
...
function pickGuess( wordInfo, history ) {
  ...
  // EDIT BELOW THIS
  return wordInfo.magic[history.count - 1];
  // EDIT ABOVE THIS
}
